...
def select_bin(IQ):   #生成每个bin的相位和幅度信号
    ...
    for i in range(I.shape[0]):
        ...
        trendI = trendI[5:]
        trendI = gaussian_filter1d(trendI, sigma=3)
        trendQ = trendQ[5:]
        trendQ = gaussian_filter1d(trendQ, sigma=3)
...
def fmcw_pro(file_path, file, audio_type='wav'):
    ...
    I_mics = []
    Q_mics = []
    for i in range(original_data_mc.shape[1]):
        arrayIQ.append(gen_IQ(original_data_mc[int(48000*1):, i], return_data))  # 生成2个麦克风的IQ信号
    ...
    #         bins = [i * C / (2 * B) for i in range(am_map[:, :].shape[0])]
        # plt.figure()
        # plt.pcolormesh(tt, bins, np.abs(Q_diff[:, :]), vmin=None, vmax=np.max(np.abs(Q_diff[:, :]))/2)
        # plt.colorbar()
        # letter_size = 15
...
if __name__ == '__main__':
    file_path = 'G:/experiments/2023/far_lip/farlip/rawdata/'
    files = os.listdir(file_path)
    for file in files:
        pattern = re.compile(r'\d+')
        res = re.findall(pattern, file)
        if (len(res) == 3 and int(res[1]) <= 200 and int(res[0]) >= 0 and file[-3:] == 'pcm'):
