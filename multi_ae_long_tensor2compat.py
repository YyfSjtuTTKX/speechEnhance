...
sess = tf.compat.v1.InteractiveSession()  # 创建session
...
# dcay改为momentum
def bn_layer(inputs, training, scope=None):
    return tf.keras.layers.BatchNormalization(momentum=0.9, scale=True)(inputs, training=training)
...
def dcnn(inputs_sense,inputs_voice, train, keep_prob, reuse=False, name='cnnBN'):
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        econv1_voice = tf.compat.v1.layers.conv2d(inputs_voice, 16, (9, 5), dilation_rate=(1, 1), padding='same')
        ...
        econv2_voice = tf.compat.v1.layers.conv2d(eh_pool1_voice, 32, (9, 5), dilation_rate=(2, 2), padding='same')
        BN_out2 = bn_layer(econv2_voice,train, scope='eBN2')
        eh_conv2_voice = tf.nn.relu(BN_out2)
        eh_pool2_voice = tf.nn.max_pool2d(input=eh_conv2_voice, ksize=[1, 1, 2, 1], strides=[1, 1, 2, 1], padding='SAME')

        econv3_voice = tf.compat.v1.layers.conv2d(eh_pool2_voice, 64, (5, 5), dilation_rate=(2,2), padding='same')
        BN_out3 = bn_layer(econv3_voice, train, scope='eBN3')
        eh_conv3_voice = tf.nn.relu(BN_out3)
        eh_pool3_voice = tf.nn.max_pool2d(input=eh_conv3_voice, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

        econv4_voice = tf.compat.v1.layers.conv2d(eh_pool3_voice, 64, (5, 5), dilation_rate=(1,1), padding='same')
        ...
        econv5_voice = tf.compat.v1.layers.conv2d(eh_pool4_voice, 64, (5, 5), dilation_rate=(2, 2), padding='same')
        ...
        econv6_voice = tf.compat.v1.layers.conv2d(eh_pool5_voice, 4,  (5, 5), dilation_rate=(1, 1), padding='same')
        ...
        conv1_sense = tf.compat.v1.layers.conv2d(inputs_sense, 16, (9, 5), dilation_rate=(1,1), padding='same')
        BN_out1_sense = bn_layer(conv1_sense, train, scope='BN1_sense')
        h_conv1_sense = tf.nn.relu(BN_out1_sense)
        h_pool1_sense = tf.nn.max_pool2d(input=h_conv1_sense, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')


        conv2_sense = tf.compat.v1.layers.conv2d(h_pool1_sense, 16, (5, 5), dilation_rate=(2, 2), padding='same')
        BN_out2_sense = bn_layer(conv2_sense, train, scope='BN2_sense')
        h_conv2_sense = tf.nn.relu(BN_out2_sense)
        h_pool2_sense = tf.nn.max_pool2d(input=h_conv2_sense, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')


        conv3_sense = tf.compat.v1.layers.conv2d(h_pool2_sense, 32, (5, 5), dilation_rate=(1, 1), padding='same')
        BN_out3_sense = bn_layer(conv3_sense, train, scope='BN3_sense')
        h_conv3_sense = tf.nn.relu(BN_out3_sense)
        h_pool3_sense = tf.nn.max_pool2d(input=h_conv3_sense, ksize=[1, 1, 2, 1], strides=[1, 1, 2, 1], padding='SAME')


        conv4_sense = tf.compat.v1.layers.conv2d(h_pool3_sense, 32, (5, 5), dilation_rate=(4, 1), padding='same')
        ...
        conv5_sense = tf.compat.v1.layers.conv2d(h_pool4_sense, 16, (5, 5), dilation_rate=(2, 1), padding='same')
        ...
        h_sense = tf.image.resize(h_pool5_sense, (eh_pool6_voice.shape[1], eh_pool6_voice.shape[2]), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        print(h_sense.shape)
        ...
        print(nchannel)
        print("ch_pool.shape",ch_pool.shape)
        ...
        rconv1 = tf.compat.v1.layers.conv1d(ch_pool, nchannel, 5, strides=1, padding='same', dilation_rate=1)
        rBN_out1 = bn_layer(rconv1, train, scope='rBN1')
        rh_conv1 = tf.nn.relu(rBN_out1)
        rh_pool1 = rh_conv1 + rconv1

        rconv2 = tf.compat.v1.layers.conv1d(rh_pool1, nchannel, 5, strides=1, padding='same', dilation_rate=2)
        rBN_out2 = bn_layer(rconv2, train, scope='rBN2')
        rh_conv2 = tf.nn.relu(rBN_out2)
        rh_pool2 = rh_conv2

        rconv3 = tf.compat.v1.layers.conv1d(rh_pool2, nchannel, 5, strides=1, padding='same', dilation_rate=1)
        rBN_out3 = bn_layer(rconv3, train, scope='rBN3')
        rh_conv3 = tf.nn.relu(rBN_out3)
        rh_pool3 = rh_conv3 + rh_pool2

        rconv4 = tf.compat.v1.layers.conv1d(rh_pool3, nchannel, 5, strides=1, padding='same', dilation_rate=2)
        rBN_out4 = bn_layer(rconv4, train, scope='rBN4')
        rh_conv4 = tf.nn.relu(rBN_out4)
        rh_pool4 = rh_conv4 + rh_pool3

        rconv5 = tf.compat.v1.layers.conv1d(rh_pool4, nchannel, 5, strides=1, padding='same', dilation_rate=1)
        rBN_out5 = bn_layer(rconv5, train, scope='rBN5')
        rh_conv5 = tf.nn.relu(rBN_out5)
        rh_pool5 = rh_conv5 + rh_pool4

        rconv6 = tf.compat.v1.layers.conv1d(rh_pool5, nchannel, 5, strides=1, padding='same', dilation_rate=2)
        rBN_out6 = bn_layer(rconv6, train, scope='rBN6')
        rh_conv6 = tf.nn.relu(rBN_out6)
        rh_pool6 = rh_conv6 + rh_pool5

        rconv7 = tf.compat.v1.layers.conv1d(rh_pool6, nchannel, 5, strides=1, padding='same')
...
xs_sense = tf.compat.v1.placeholder(tf.float32, [None, Xtest_sense.shape[1], Xtest_sense.shape[2], 6])
xs_voice = tf.compat.v1.placeholder(tf.float32, [None, Xtest_voice.shape[1], Xtest_voice.shape[2], 1])
ys = tf.compat.v1.placeholder(tf.float32, [None, Xtest_voice.shape[1], Xtest_voice.shape[2], 1])
...
loss_sum = tf.reduce_sum(input_tensor=tf.square(logits-ys))
cross_entropy = loss_sum
global_step = tf.Variable(0, trainable=False)
train_step = tf.compat.v1.train.AdamOptimizer().minimize(cross_entropy, global_step=global_step)
        ...
        for i in range(60000):
           ...
           if i % 200 == 0:
               ...
               if testloss < best_loss:
                   best_loss = testloss
                   best_step = i
                   save_path = saver.save(sess, './save3/savenet.ckpt')
               nowtime = time.time()
