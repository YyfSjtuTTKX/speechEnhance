...
def fmcw_pro(file_path, file, audio_type='wav'):
    ...
    for i in range(original_data_mics_new.shape[1]):
        arrayIQ.append(gen_IQ(original_data_mics_new[int(24000*3):int(24000*-2), i], return_data))  # 生成2个麦克风的IQ信号
        am_map, phase_map, phase_map_undiff = select_bin(arrayIQ[-1])  # 选择第一个麦克风所有bin的幅度信号进行画图
    ...
    #     plt.plot(diff_phase[i*16],'.-')
        ...
        plt.figure()
        plt.pcolormesh(tt, bins, np.abs(am_map[:, :]), vmin=None, vmax=np.max(np.abs(am_map[:, :]))/2)
        plt.colorbar()
        letter_size = 15
    ...
    # file_path_new = 'G:/experiments/2023/prp/se/data_pro/'
...
if __name__ == '__main__':
    file_path = '/home/yangyf/pythonWorkspace/SpeechEnhancement/SpeechEnhancement/se/data/'
    file = '0_0.wav'
